...
data <- read.csv('C:/Users/lisan/OneDrive/Escritorio/MAESTRIA/EEA/TP-2/eea/TP-2/bono.csv') %>% 
  ...
  filter(fecha > '2020-10-01' & fecha < '2022-12-01') %>% 
  as_tsibble(index = fecha)

data_ts <- ts(data$volumen, deltat = 1/12)
...
data_sem <- read.csv('C:/Users/lisan/OneDrive/Escritorio/MAESTRIA/EEA/TP-2/eea/TP-2/bono.csv') %>%
  select(ds = fecha, y = volumen) %>% 
  mutate(ds = as.Date(ds)) %>% 
  group_by(semana = floor_date(ds, unit = "week")) %>% 
...
```{r autocorrelacion}
acf(data_sem,type = "correlation", na.action = na.pass )
```
...
Vamos a utilizar un modelo auto arima
...
data <- read.csv('C:/Users/lisan/OneDrive/Escritorio/MAESTRIA/EEA/TP-2/eea/TP-2/bono.csv') %>%
  select(fecha, volumen) %>% rename(., ds=fecha, y=volumen)
...
```{r}
prophet_plot_components(prophet, fcst=predict(prophet, data_dia))
```
...
```{r}
cv_base = cross_validation(prophet, initial = 366, period = 7, horizon = 15, units = 'days')
```
